...
import { canChangeThreadWorkspaceWithoutConfirmation } from '@yachiyo/shared/threadWorkspaceRules'
import { hasPendingPlanDocument } from '@yachiyo/shared/planMode'
...
export class YachiyoServerThreadDomain {
  ...
  async createBranch(input: { threadId: string; messageId: string }): Promise<ThreadSnapshot> {
    ...
    const branchThread = withThreadCapabilities({
      ...
      ...(thread.enabledTools ? { enabledTools: thread.enabledTools } : {}),
      ...(thread.runMode ? { runMode: thread.runMode } : {}),
