...
async function workspaceWith(fileNames: string[]): Promise<string> {
...
test('a destination that decodes to a NUL byte does not break the whole batch', async () => {
  ...
  const workspacePath = await workspaceWith(['ordinary.md'])
...
test('a file literally named with a percent escape still resolves', async () => {
  // The raw reading is what keeps these reachable; dropping the decoded
  // candidate must not take the literal one with it.
  const workspacePath = await workspaceWith(['a%00.md'])
...
test('an ordinary encoded destination resolves through the real resolver', async () => {
  // The layer the rest of this feature's tests stubbed out.
  const workspacePath = await workspaceWith(['a b.md', 'c#d.md'])
...
test('a destination whose decoding would be renamed never opens the renamed file', async () => {
  ...
  // wrong one.
  const workspacePath = await workspaceWith(['package.json'])
...
test('an unencoded reference to that same file still resolves', async () => {
