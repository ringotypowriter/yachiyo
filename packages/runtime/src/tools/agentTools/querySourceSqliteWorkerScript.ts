...
function parseSourceEventSourceRowId(rowId) {
  const prefix = 'source_event:'
  return String(rowId).startsWith(prefix) ? String(rowId).slice(prefix.length) : undefined
}

function getThreadIdFromRowId(rowId) {
  if (!rowId) return undefined
  const parsed = parseRowId(rowId)
  return parsed.kind === 'thread' && parsed.parts.length === 1 ? parsed.parts[0] : undefined
}

function getFolderIdFromRowId(rowId) {
  if (!rowId) return undefined
  const parsed = parseRowId(rowId)
...
function buildSpanSearchText({ matchedEvidence, messages, thread }) {
  return [
    thread.title,
    thread.preview,
    ...matchedEvidence,
...
function createIdfByTerm(terms, documents) {
...
function calculateCoverage({ idfByTerm, searchText, terms }) {
  const totalWeight = terms.reduce((sum, term) => sum + (idfByTerm.get(term) ?? 0), 0)
  if (totalWeight === 0) return 0

  const coveredWeight = terms.reduce(
    (sum, term) => (includesText(searchText, term) ? sum + (idfByTerm.get(term) ?? 0) : sum),
    0
  )
  return coveredWeight / totalWeight
}

function calculateDensity({ idfByTerm, messages, terms }) {
  const content = messages.map((message) => message.content).join('\\n')
  const weightedOccurrenceCount = terms.reduce(
    (sum, term) => sum + countTermOccurrences(content, term) * (idfByTerm.get(term) ?? 0),
    0
  )
  return weightedOccurrenceCount / Math.max(1, messages.length)
}
...
function sigmoidNormalizeHigher(values) {
  const mean = values.reduce((sum, value) => sum + value, 0) / values.length
  const variance = values.reduce((sum, value) => sum + (value - mean) ** 2, 0) / values.length
  const standardDeviation = Math.sqrt(variance)
  if (standardDeviation === 0) return values.map(() => 0.5)
  return values.map((value) => sigmoid((value - mean) / standardDeviation))
}

function sigmoidNormalizeLower(values) {
  const mean = values.reduce((sum, value) => sum + value, 0) / values.length
  const variance = values.reduce((sum, value) => sum + (value - mean) ** 2, 0) / values.length
  const standardDeviation = Math.sqrt(variance)
  if (standardDeviation === 0) return values.map(() => 0.5)
...
function rankThreadSpanCandidates(candidates, queryTerms) {
  if (candidates.length === 0) return []
  const terms = uniqueValues(queryTerms)
  if (terms.length === 0) return candidates.map((candidate) => candidate.row)
  ...
  const densityScores = sigmoidNormalizeHigher(
    candidates.map((candidate) => candidate.density(idfByTerm))
  )
...
function clusterMatchedIndexes(indexes) {
...
function createThreadSpanCandidate({ bm25, matchedEvidence, messages, ordinal, queryTerms, thread, view }) {
...
function getThreadSpanContextRadius(view) {
...
function queryTextThreadSpans(db, input, text) {
  ...
  const queryTerms = tokenizeQuery(text)
  const candidates = []
  let ordinal = 0
  for (const threadId of orderedThreadIds) {
    ...
    const matchedIndexes = messages
      ...
      .filter(
        ({ message }) =>
          matchesByMessageId.has(message.id) && isTimestampInRange(message.createdAt, where)
      )
      .map(({ index }) => index)
    ...
    if (matchedIndexes.length > 0) {
      for (const cluster of clusterMatchedIndexes(matchedIndexes)) {
        ...
        const clusterMatches = matches.filter((match) => clusterMessageIds.has(match.messageId))
        const bm25 = Math.min(...clusterMatches.map((match) => match.bm25))
        const candidate = createThreadSpanCandidate({
          bm25,
          matchedEvidence: clusterMatches.map((match) => extractSnippet(match.content, text)),
      ...
      const rangeMessages = messages
        .filter((message) => isTimestampInRange(message.createdAt, where))
        .slice(0, contextRadius * 2 + 1)
      const candidate = createThreadSpanCandidate({
        bm25: titleBm25ByThread.get(threadId) ?? ordinal,
        matchedEvidence: [],
        messages: rangeMessages,
        ordinal,
        queryTerms,
  ...
  const rows = rankThreadSpanCandidates(candidates, queryTerms)
  const orderedRows =
    input.orderBy === 'timeAsc' || input.orderBy === 'timeDesc'
      ? sortRows(rows, input.orderBy)
      : rows
  return paginate(orderedRows, input)
}

function queryRowIdThreadSpan(db, input) {
  const where = input.where ?? {}
  if (!where.rowId) return { rows: [] }
  const span = parseSpanRowId(where.rowId)
  if (!span) return { rows: [] }
  ...
  const messages = findMessageRange(
    fetchAllThreadMessages(db, span.threadId),
    span.startMessageId,
    span.endMessageId,
    (input.view ?? 'index') === 'detail' ? DETAIL_CONTEXT_RADIUS : 0
  )
  const row = buildThreadSpanRow({
    messages,
    thread,
...
function queryThreadSpans(db, input) {
  if ((input.view ?? 'index') === 'detail' && input.where?.rowId) {
    return queryThreadMessages(db, {
      ...input,
  ...
  if (input.where?.rowId) return queryRowIdThreadSpan(db, input)
...
function queryThreads(db, input) {
  const where = input.where ?? {}
  const view = input.view ?? 'index'
  const threadId = where.threadId ?? getThreadIdFromRowId(where.rowId)
  if (threadId && view === 'content') {
    return queryThreadSpans(db, {
      ...input,
      from: 'thread_spans',
      view: 'content',
      where: {
...
function queryThreadFolders(db, input) {
  const where = input.where ?? {}
  const view = input.view ?? 'index'
  const folderId = where.folderId ?? getFolderIdFromRowId(where.rowId)
...
function querySourceEvents(db, input) {
  if (input.where?.rowId && input.view && input.view !== 'index') {
    ...
    if (parsed.kind === 'activity_record') {
      return queryActivityRecords(db, {
        ...input,
