...
final class ResponseViewSizingTests: XCTestCase {
    func testAssistantRowHeightMatchesRenderedMarkdownAtPhoneWidths() throws {
        ...
        let message = MessageListView.MessageRepresentation(
            id: "assistant", createdAt: Date(), role: .assistant,
            ...
            """, count: 3),
            isRevealed: false, isThinking: false, thinkingDuration: 0
        )
        let entry = MessageListView.Entry.responseContent(message.id, message)
        ...
        for width: CGFloat in [320, 390, 430] {
            ...
            row.markdownView.setMarkdownManually(list.markdownPackageCache.package(for: message, theme: list.markdownTheme))
