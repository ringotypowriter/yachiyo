...
def build_applescript():
  ...
  repeat with cal in calendars
    set out to out & (title of cal) & linefeed
  end repeat
  return out
end tell'''
...
def main():
    parser = argparse.ArgumentParser(description='List calendars in Calendar.app')
    ...
    if args.json:
        out['calendars'] = [line.strip() for line in result.stdout.strip().splitlines() if line.strip()]
        print(json.dumps(out, indent=2))
