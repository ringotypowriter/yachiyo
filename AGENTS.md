...
- Do not add or update unit tests solely for system prompts, text labels, descriptive copy, or other static wording. Verify those changes by reading the edited text or reviewing the diff instead. Add tests only when the wording change affects executable behavior, parsing, state transformation, or another real contract.
